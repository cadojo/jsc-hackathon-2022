...
# ╔═╡ 25a3594e-61d0-4533-a117-8d9787438966
begin
...
# ╔═╡ 5e01f7a6-06cf-463b-9528-cc317567edf3
...
# ╔═╡ 025bf930-e1dc-4991-ace8-64ff70fb2a9e
md"""
# Welcome! 👋
...
# ╔═╡ 28a2f730-7ee6-11ec-0d91-5f60b9d53c13
md"""
...
# ╔═╡ dc1c8329-d399-474c-977f-e45a9954db37
md"""
## Julia is Dynamic
...
# ╔═╡ d1085fc9-c981-472a-a222-622d0014269e
let # Julia is simple, like Python
	world_id = 42
	@with_terminal print("Hello, world #", world_id)
end

# ╔═╡ eb3f211f-bd69-47fc-8223-4f3adc056b3a
md"""
## Julia is Fast
...
# ╔═╡ 6bd7d5dc-6f88-4055-920f-9466d533faef
let
...
# ╔═╡ 78ca5e54-1fa6-492b-874e-6c148555e50d
md"""
## Julia is Composable
...
# ╔═╡ 2fd25613-4494-4cdc-bfcf-3a3e6a67602e
md"""
I can use functions **I wrote** on types **written by someone else**. Why is that?
"""

# ╔═╡ 31974f06-03c2-4b3b-b9cb-2062d5da60a2
md"""
#### Julia Separates Types & Methods
"""

# ╔═╡ 4f958888-c1ca-49c1-aa0e-a23f47b8de17
struct MyType
	field
end

# ╔═╡ 0dd06d14-9088-41f0-a362-deebe332334d
function myfunction(arg::MyType)
	return arg.field
end;

# ╔═╡ c0fcad43-04ac-4e5b-b2de-358d3ddc799e
md"""
...
# ╔═╡ e6df19de-3735-4ea0-938f-edac1a299bdd
md"""
...
# ╔═╡ 684d6fd2-4895-4c25-9ef7-c5d93dd97a2d
# All of the steps above are involved in this code & output!
@with_terminal begin
	print("Hello, world!")
end

# ╔═╡ 99cf9d2f-1ee2-4da1-b44f-40634dee7969
md"""
## Digital Logic (Math)
...
# ╔═╡ 0bda709d-ec32-4037-8318-327f40e8bed9
md"""
...
# ╔═╡ 53ee5ed8-a969-4ef8-a0c6-cb95adc5a145
md"""
## Computer Chips
...
# ╔═╡ 1659b93d-5b10-4472-9725-4c9d9a1c9f87
md"""
## Assembly Language
...
# ╔═╡ 0b2aec98-2f25-445c-86ef-1a7116592920
md"""
...
# ╔═╡ 69ed5559-62c5-43f3-a6b8-b53e8d7ceeff
# What are some other programming languges?
programming_languages = [];

# ╔═╡ 2a822fd3-8749-4303-9a59-7a5f4e956be2
md"""
## Summary
_Let's re-cap._
...
# ╔═╡ ec7b33ac-0246-4543-8209-ea22384e2f70
md"""
...
# ╔═╡ e4e6813c-59cd-4c71-bd68-a6b25717413a
md"""
...
# ╔═╡ b59d4510-6dfd-4930-aaa5-a6bbbdc574f8
...
# ╔═╡ c7fb7d8c-e610-4fde-a017-13a25768c73d
md"""
## Why JIT Compilation?
...
# ╔═╡ b36302d4-b46a-43e0-be44-65c0dc9cfad4
@with_terminal let
...
# ╔═╡ 90ef754e-5c14-41d7-97ab-518778bcdeb7
md"""
## Manual JIT Compilation
_Users can take advantage of this explicitly!_
"""

# ╔═╡ a9077987-5bc5-45d7-addd-0fb0de1e30ea
@with_terminal let

	"""
	Scalar multiplication.
	"""
	multiply(arg1::Number, arg2::Number) = print("Scalar Multiplication: ", arg1*arg2)

	"""
	Matrix multiplication.
	"""
	multiply(arg1::Matrix, arg2::Matrix) = print("Matrix Multiplication: ", arg1*arg2)
...
# ╔═╡ cfcc1690-e0cf-49ad-927e-d743ddedc8c8
md"""
## Custom Types
...
# ╔═╡ b9b5070a-3df3-4653-83a8-0af5a7ee7632
"""
Computes dynamic pressure.
"""
dynamicpressure(ρ, v) = 1/2 * ρ * v * v;

# ╔═╡ 178c1dff-10b7-40e1-bb60-81c18951e32b
begin
	
	let ρ = 1.225u"kg/m^3", v = 1.0u"km/s"
		upreferred(dynamicpressure(ρ, v))
	end
	
end

# ╔═╡ dd21bcb1-fef3-41c3-94c5-2f5ee142c193
let
	...
	let ρ = MyNumber(5.0), v = MyNumber(10.0)
		dynamicpressure(ρ,v)
	end
	
end

# ╔═╡ b037a4e3-2107-44a9-8eaf-92502fa93087
md"""
## Type Tree
...
# ╔═╡ 0af1c764-f3c3-4906-8d99-e106d81f94ce
supertypes(MyNumber)

# ╔═╡ 353d7a07-4f81-43d5-96ce-e074d0b7623c
isabstracttype(Number)

# ╔═╡ 87bbb4a2-2a87-48f2-8c3b-d5c8ee577ce2
isconcretetype(MyNumber)

# ╔═╡ a3b2b7ca-0ea0-4fe3-aabe-72c905265c2a
isconcretetype(MyNumber{Int})

# ╔═╡ 27c716b9-9d99-4de1-b781-e677dae305f1
md"""
## Julia Syntax
_How does Julia code syntax compare to other languages?_
"""

# ╔═╡ 8a249bf4-5506-48f3-aef2-2a6fcf173abe
let

	# Interactive! What kinds of things can you do in MATLAB / Python / etc?
...
# ╔═╡ 6357d4a9-f4d6-427a-835a-fe333c3ffcfb
md"""
# Julia Resources
_Where to look for more information!_
"""

# ╔═╡ ea717dd3-fc97-44e2-a073-007b22ca43f3
md"""
## YouTube
_There are lots of helpful tutorials on there!_
"""

# ╔═╡ aa0c7efb-28ca-4933-ab27-93961458f7c3
html"""
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/videoseries?list=PLP8iPy9hna6SCcFv3FvY_qjAmtTsNYHQE" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
"""

# ╔═╡ 2148d759-dc63-42c6-94b1-3676c190b378
md"""
## Documentation
_Official language documentation._
"""

# ╔═╡ c09fa83f-cdb9-46a5-b67c-7b83440b78b9
html"""
<iframe width="660" height="500" src="https://docs.julialang.org/en/v1/" 
</iframe>
"""

# ╔═╡ e9c7094c-aeee-4cf2-acd5-174e31734fb8
md"""
## Discourse
_A forum filled with friendly users. We want to help!_
"""

# ╔═╡ 48a4bdc6-74d4-4659-953d-f56e5cd9d1f3
html"""
<iframe width="660" height="500" src="https://discourse.julialang.org/sdfg" 
</iframe>
"""

# ╔═╡ 7f52731b-0de2-4a89-a1c8-82ede329b228
md"""
# Supporting Code
"""

# ╔═╡ 157e0251-c750-4edc-a23c-82d0b1ea57d8
md"""
## Dependencies
